...
const acceptRequest = async (req, res) => {
  try {
    ...
    const { requestType, targetAgentId, requesterId, shiftId, availableSlots } =
      request;
    ...
    if (!acceptingAgent)
      return res
        .status(404)
        .json({ error: "L'agent qui accepte n'existe pas." });
    ...
    if (targetAgentId && targetAgentId.toString() !== agentId) {
      return res
        .status(403)
        .json({ error: "Vous n'êtes pas l'agent cible de cette demande." });
    }
    ...
    if (requestType === "Swap") {
      if (!selectedSlot)
        return res
          .status(400)
          .json({ error: "Un créneau doit être sélectionné pour un swap." });
    ...
    if (requestType === "Replacement") {
      const existingShift = await Shift.findOne({
        ...
        $or: [
          {
            startDate: { $lt: shiftId.endDate },
            endDate: { $gt: shiftId.startDate },
          },
        ],
      });
      if (existingShift)
        return res
          .status(400)
          .json({ error: "Vous avez déjà un shift à ce moment-là." });

      // Calculer la durée du shift en heures
      const shiftDurationMs =
        new Date(shiftId.endDate) - new Date(shiftId.startDate);
      const shiftDurationHours = shiftDurationMs / (1000 * 60 * 60); // Convertir en heures

      // Mettre à jour la balance des agents
      await Agent.findByIdAndUpdate(requesterId, {
        $inc: { balance: -shiftDurationHours },
      });
    ...
    await Notification.insertMany([
      {
        ...
        type: "Status Update",
        message: `Votre demande de ${requestType.toLowerCase()} a été acceptée par ${
          acceptingAgent.code
        }.`,
      },
      {
        ...
        type: "Status Update",
        message: `Vous avez accepté un ${requestType.toLowerCase()} avec ${
          requesterAgent.code
        }.`,
      },
    ]);
    ...
    res
      .status(500)
      .json({ error: "Erreur serveur lors de l'acceptation de la demande." });
  }
};
